...
const TextManage = ({clickEvt, color, stroke}) => {
    ...
    // useEffect(() => {}, [textRef, numberRef, dotinfo])
    ...
    return (
        <div className='stamp_dot_text' 
            style={stroke ?
                {pointerEvents: "none", color: `${color}`, 
        ...
        >
            ...
            {dotinfo.style =="B" &&
                <>
                    <div className='stamp_b_name' onClick={() => clickEvt("name")}>{dotinfo.name}</div>
                    <div className='stamp_b_num1' onClick={() => clickEvt("num1")}>{dotinfo.num1}</div>
                    <div className='stamp_b_num2' onClick={() => clickEvt("num2")}>{dotinfo.num2}</div>
                    <div className='stamp_b_num3' onClick={() => clickEvt("num3")}>{dotinfo.num3}</div>
                </>
            }
            {dotinfo.style =="C" &&
                <>
                    <div className='stamp_c_div stamp_c_top' onClick={() => clickEvt("name")} ref={textRef}>{dotinfo.name}</div>
                    <div className='stamp_c_div stamp_c_bottom' onClick={() => clickEvt("number")} style={{position:"absolute", bottom: "0", left: "50%"}} ref={numberRef}>{dotinfo.number}</div>
                </>
            }
            {dotinfo.style =="D" &&
                <>
                    <div className='stamp_c_div stamp_c_top' onClick={() => clickEvt("name")} ref={textRef}>{dotinfo.name}</div>
                    <div className='stamp_c_div stamp_c_bottom' onClick={() => clickEvt("cont")} style={{position:"absolute", bottom: "0", left: "50%"}} ref={numberRef}>{dotinfo.cont}</div>
                </>
            }
        </div>
    );
